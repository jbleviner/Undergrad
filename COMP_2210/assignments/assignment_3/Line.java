...
public class Line implements Comparable<Line>, Iterable<Point> {
    ...
    * Collection c.
    * 
    * @param c (Collection) a collection of Points
    */
    ...
    * @return true/false (boolean) returns a boolean
    */
   public boolean add(Point p) {
      if (line.isEmpty() || line.size() == 1) {
         line.add(p);
         return true;
      }
      if (line.contains(p)) {
      ...
      double firstSlope = line.first().slopeTo(p);
      double lastSlope = line.last().slopeTo(p);
    ...
    * 
    * @param that (Line) a line to compare to the invoking line
    * @return cmp (int) an int representation of the comparison betweeen this line and that line
    */
   @Override
   public int compareTo(Line that) {   
      ...
      int cmp = thisFirst.compareTo(thatFirst);
      if (cmp == 0) {
         cmp = thisLast.compareTo(thatLast);
      }
      return cmp;
   }
    ...
    * 
    * @return itr (Iterator) an iterator for Line
    */
   @Override
    ...
    * @return boolean returns true if invoking object equals obj
    * THIS METHOD IS PROVIDED FOR YOU AND MUST NOT BE CHANGED.
    */
   @Override 
